...
namespace MyCore
{
    ...
    public class MyAnalyse
    {
        ...
        {
            double t = range_from_psi_hob(kt, psi, ft) * Utils.Const.FT2MI;
            return t * Utils.Const.MI2M;
        }
        ...
        public double CalcNuclearRadiationRadius(double kt, double hob_ft, double rem)
        {
            ...
            {
                t = ground_range_from_slant_range(t, hob_ft * Utils.Const.FT2MI);
            }
        ...
        {
            //miles 转成 m
            return fireball_radius(kt, airburst) * Utils.Const.MI2M;
        }
        ...
        // 放射性尘埃,返回的maximumDownwindDistance和maximumWidth是公里
        ...
        private List<Coor> plot_fallout(double lng, double lat, List<Coor> points, double rad)
        {
            ...
            {
                const double mi2km = 1.60934;
                double gzlat = lat + Utils.Const.RAD2DEG * (points[i].lat * mi2km / R);
                double gzlng = lng + Utils.Const.RAD2DEG * (points[i].lng * mi2km / R / Math.Cos(Utils.Const.DEG2RAD * (lat)));
        ...
        private List<Coor> SFSS_fallout_points(Dictionary<string, double> f, double angle, int steps)
        {
            ...
            var stem_inner_x = Math.Sin(80 * Utils.Const.DEG2RAD) * stem_circle_radius;
        ...
        private void rotate_points(ref List<Coor> points_array, double angle_degrees)
        {
            ...
            //rotate
            double angle_rad = (angle_degrees) * Utils.Const.DEG2RAD;
        ...
        private Dictionary<int, Dictionary<string, double>> SFSS_fallout(double kt, int[] rad_doses, double fission_fraction, double windspeed)
        {
            ...
            for (var i = 0; i < rad_doses.Length; i++)
            {
                ...
                d["x_center"] = p["x2"] * Utils.Const.FT2MI;
                d["upwind_stem_distance"] = log_lerp(p["x1"], p["i1"], p["x2"], p["i2"], rad) * Utils.Const.FT2MI;
                d["downwind_stem_distance"] = log_lerp(p["x3"], p["i3"], p["x4"], p["i4"], rad) * Utils.Const.FT2MI;
                d["max_stem_width"] = log_lerp(0, p["i2"], p["ys"], 1, rad) * Utils.Const.FT2MI;
                d["upwind_cloud_distance"] = (rad > p["i6"] ? log_lerp(p["x6"], p["i6"], p["x7"], p["i7"], rad) : log_lerp(p["x5"], p["i5"], p["x6"], p["i6"], rad)) * Utils.Const.FT2MI;
                d["downwind_cloud_distance"] = log_lerp(p["x7"], p["i7"], p["x9"], p["i9"], rad) * Utils.Const.FT2MI;
                d["max_cloud_width"] = (p["y8"] * log10(p["i7"] / rad)) / (log10(p["i7"] / p["i9"])) * Utils.Const.FT2MI;
                d["cloud_widen_point"] = (
                        p["x7"] + (p["x8"] - p["x7"]) * (
        ...
        private double[] destination_from_bearing(double lat, double lon, int bearing, double distance)
        {
            ...
            var R = 6371; // km
            var d = distance;
            var lat1 = Utils.Const.DEG2RAD * lat;
            var lon1 = Utils.Const.DEG2RAD * lon;
            var brng = Utils.Const.DEG2RAD * bearing;

            var lat2 = Math.Asin(Math.Sin(lat1) * Math.Cos(d / R) +
                                Math.Cos(lat1) * Math.Sin(d / R) * Math.Cos(brng));
            var lon2 = lon1 + Math.Atan2(Math.Sin(brng) * Math.Sin(d / R) * Math.Cos(lat1),
                                     Math.Cos(d / R) - Math.Sin(lat1) * Math.Sin(lat2));
            return new double[] { Utils.Const.RAD2DEG * lat2, Utils.Const.RAD2DEG * lon2 };
        }
	    ...
	    */
        ...
        private double range_from_psi_hob_1kt_interpolated(double psi, double hob)
        {
            ...
            while (intercept.lng < h)
            {
                var rng_at_prop = lerp(rngs[p1][outer_index], 1, intercept.lat, 0, proportion);
                var hob_at_prop = lerp(hobs[p1][outer_index], 1, intercept.lng, 0, proportion);
                if (hob_at_prop < h)
                {
        ...
        private Coor getLineLineIntersection(int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
        {
            ...
            double x = (getLineLineIntersection_det(det1And2, x1LessX2,
                  det3And4, x3LessX4) /
                  det1Less2And3Less4);
            double y = (getLineLineIntersection_det(det1And2, y1LessY2,
                  det3And4, y3LessY4) /
                  det1Less2And3Less4);
            return new Coor(x, y);
        }

        private double getLineLineIntersection_det(double a, double b, double c, double d)
        {
            return a * d - b * c;
        }
